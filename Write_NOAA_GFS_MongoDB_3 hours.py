...
def parse_data_rez_df(output_dir):
    ...
    id_stations = list(map(int, df['id_st'].unique()))
    
    result_df = pd.DataFrame()
    
    for id_station in id_stations:
        df_1 = df[df['id_st'] == id_station].reset_index()
        df_1 = df_1.set_index('dt')
        df_1 = df_1[~df_1.index.duplicated()]
        rez_df = df_1.resample('1H').interpolate()
        rez_df.replace([np.inf, -np.inf], np.nan, inplace=True)
        rez_df = rez_df.drop('index', axis=1)
        rez_df['dt'] = pd.to_datetime(rez_df.index)
        rez_df = rez_df.reset_index(drop=True)
        result_df = pd.concat([result_df, rez_df], axis=0)
...
def write_to_baza():
    ...
    meteo_df_all = parse_data_rez_df('D:\\ANACONDA\\NOAA\\GFS\\3-hour files')
    meteo_df_all['id_st'] = meteo_df_all['id_st'].astype(int)
...
def schedule_job():
    schedule.every().day.at("10:59").do(run_main)
